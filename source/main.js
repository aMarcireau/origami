...
function secureCookies() {
    const session = electron.session.fromPartition("persist:origami");
    return session.cookies.get({}).then(cookies =>
        Promise.all(
            cookies
        ...
        fs.readFile(colorsFilename, (colorsError, colors) => {
            ...
            if (colorsError) {
                electron.dialog.showErrorBox(
                    "Configuration error",
                    `'${__dirname}/colors.json' could not be open for reading`
                );
                ...
                try {
                    ...
                    for (const key of [
                        ...
                        if (
                            !(key in parsedColors) ||
                            !/^#[a-zA-Z0-9]{6}$/.test(parsedColors[key])
                        ) {
                            electron.dialog.showErrorBox(
                                "Configuration error",
                                `'${key}' is not a color in '${__dirname}/colors.json'`
                            );
                            colorsAreValid = false;
                            break;
                        }
                    ...
                    if (
                        ...
                        electron.dialog.showErrorBox(
                            "Configuration error",
                            `Unexpected keys ${Object.keys(parsedColors)
                                .map(key => `'${key}'`)
                                .join(", ")} in '${__dirname}/colors.json'`
                        );
                        colorsAreValid = false;
                    }
                } catch (error) {
                    ...
                    electron.dialog.showErrorBox(
                        "Configuration error",
                        `Parsing '${__dirname}/colors.json' failed`
                    );
                }
            }
            if (!colorsAreValid) {
            ...
            /// This is required to use GOOGLE_ABUSE_EXEMPTION in recent Chromium versions.
            electron.ipcMain.on("secure-cookies", event => {
                secureCookies()
                    .catch(console.error)
                    .finally(() => {
                        event.sender.send("secured-cookies");
                    });
            });
...
electron.app.on("ready", () => {
    const session = electron.session.fromPartition("persist:origami");
    session.protocol.registerFileProtocol("atom", (request, callback) => {
        callback({
            path: path.normalize(`${__dirname}/${request.url.substr(7)}`),
        });
    });
    session.protocol.registerFileProtocol("font", (request, callback) => {
        callback({
            path: path.join(__dirname, "fonts", request.url.substr(7)),
        });
    });
    secureCookies().catch(console.error);
        ...
        electron.Menu.setApplicationMenu(
            electron.Menu.buildFromTemplate([
                ...
                },
                {
                    label: "View",
                    submenu:
                        process.env.ORIGAMI_ENV === "development"
                            ? [
                                  { role: "toggledevtools" },
                    ...
                    submenu: [
                        {
                            ...
                            click() {
                                electron.shell.openExternal(
                                    "https://github.com/aMarcireau/origami"
                                );
                            },
                        },
                    ],
