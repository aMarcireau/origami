...
class Graph extends React.Component {
    ...
    static viewBoxFromProps(props) {
        ...
        return `${-zoomedWidth / 2 + props.xOffset} ${
            -zoomedHeight / 2 + props.yOffset
    ...
    update(nextProps) {
        ...
        for (const publication of nextProps.publications) {
            ...
            for (const node of this.nodes) {
                if (publication.doi === node.doi) {
                    ...
                    found = true;
                    const known =
                        nextProps.knownDois.has(publication.doi) ||
            ...
            if (!found) {
                ...
                if (doiToConnectedNodes === null) {
                    ...
                    const doiToNode = new Map(
                        this.nodes.map(node => [node.doi, node])
                    );
                    for (const edge of nextProps.edges) {
                        if (doiToNode.has(edge.source)) {
                            if (doiToConnectedNodes.has(edge.target)) {
                                doiToConnectedNodes
                                    .get(edge.target)
                                    .push(doiToNode.get(edge.source));
                            } else {
                                doiToConnectedNodes.set(edge.target, [
                                    doiToNode.get(edge.source),
                                ]);
                            }
                        }
                        if (doiToNode.has(edge.target)) {
                            if (doiToConnectedNodes.has(edge.source)) {
                                doiToConnectedNodes
                                    .get(edge.source)
                                    .push(doiToNode.get(edge.target));
                            } else {
                                doiToConnectedNodes.set(edge.source, [
                                    doiToNode.get(edge.target),
                                ]);
                            }
                        }
                    }
                ...
                if (publication.locked) {
                    ...
                    this.reheatSimulation = true;
                    const connectedNodes = doiToConnectedNodes.get(
                        publication.doi
                    );
                    if (connectedNodes == null) {
                        ...
                        publication.x =
                            connectedNodes[0].x + 80 * Math.cos(angle);
                        publication.y =
                            connectedNodes[0].y + 80 * Math.sin(angle);
                    } else {
                        publication.x = 0;
                        publication.y = 0;
        ...
        for (let index = this.nodes.length - 1; index >= 0; --index) {
            if (this.nodes[index].keep) {
                ...
                const newIsCiting = nextProps.doisCitingSelected.has(
                    this.nodes[index].doi
                );
                if (this.nodes[index].isCiting != newIsCiting) {
                    this.nodes[index].isCiting = newIsCiting;
                    updateRequired = true;
                }
                const newIsCited = nextProps.doisCitedBySelected.has(
                    this.nodes[index].doi
                );
        ...
        d3NodeGroup
            ...
            .text(node => {
                ...
                if (node.authors.length > 0) {
                    ...
                    for (
                        let index = authorParts.length - 1;
                        index >= 0;
                        --index
                    ) {
        ...
        this.d3Node.call(
            d3
                ...
                .on("end", () => {
                    ...
                    if (
                        (this.props.sticky || d3.event.subject.locked) &&
                        d3.event.subject.status ===
                            PUBLICATION_STATUS_IN_COLLECTION
                    ) {
                        if (!d3.event.subject.locked) {
                            d3.event.subject.locked = true;
                            this.d3Node
                                .filter(node => node === d3.event.subject)
                                .append("circle")
                                .attr("r", 30)
                                .attr("class", "locked")
                                .attr("fill", "none")
                ...
                if (d3.event.sourceEvent) {
                    if (d3.event.sourceEvent.type !== "zoom") {
                        const newZoom = Math.round(
                            (20 * Math.log(d3.event.transform.k)) / Math.log(2)
                        );
                        if (newZoom != this.props.zoom) {
                            const scale =
                                2 ** (-this.props.zoom / 20) -
                                2 ** (-newZoom / 20);
                            this.props.dispatch(
                                setGraphZoomAndOffset(
                                    newZoom,
                                    this.props.xOffset +
                                        (d3.event.sourceEvent.clientX -
                                            this.domSvg.getBoundingClientRect()
                                                .left -
                                            this.props.width / 2) *
                                            scale,
                                    this.props.yOffset +
...
export default connect(state => {
    ...
    const recommandedDois = new Set(
        Array.from(citersCountByDoi.entries())
            .filter(([doi, count]) => count >= state.graph.threshold)
            .map(([doi, count]) => doi)
    );
    const publications = Array.from(state.publications.entries())
        .filter(
            ([doi, publication]) =>
                publication.status === PUBLICATION_STATUS_IN_COLLECTION ||
                recommandedDois.has(doi)
        )
        .map(([doi, publication]) => {
            return { ...publication, doi };
        });
    const selectedDoiAndPublication = Array.from(
        state.publications.entries()
    ).find(([doi, publication]) => publication.selected);
    return {
        ...
        doisCitingSelected: new Set(
            selectedDoiAndPublication ? selectedDoiAndPublication[1].citers : []
        ),
        doisCitedBySelected: new Set(
            selectedDoiAndPublication
                ? Array.from(state.publications.entries())
                      .filter(([doi, publication]) =>
                          publication.citers.includes(
                              selectedDoiAndPublication[0]
                          )
                      )
