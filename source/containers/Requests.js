                    ...
                    <div
                        style={{
                            ...
                            height: `${
                                this.props.height -
                                40 -
                                (this.props.hasPages ? 3 : 0) -
                                (this.props.blocked ? 133 : 0)
                            }px`,
                        }}
                    >
                        <List
...
function scholarRequestToRequest(scholarRequest, state, fetching, blocked) {
    return {
        ...
        title: state.publications.has(scholarRequest.doi)
            ? state.publications.get(scholarRequest.doi).title
            : "Cancelling request",
        onTitleClick: state.publications.has(scholarRequest.doi)
            ...
            : null,
        subtitle:
            scholarRequest.type === SCHOLAR_REQUEST_TYPE_INITIALIZE
                ? "Initial request"
                : `${scholarRequest.number} / ${scholarRequest.total}`,
        color: state.publications.has(scholarRequest.doi)
            ? state.publications.get(scholarRequest.doi).selected
                ? state.colors.active
                : state.colors.link
            : state.colors.error,
        borderColor: blocked
...
function crossrefRequestToRequest(crossrefRequest, state, fetching) {
    switch (crossrefRequest.type) {
                ...
                title:
                    ...
                    CROSSREF_REQUEST_TYPE_CITER_METADATA
                        ? state.publications.has(crossrefRequest.parentDoi)
                            ? state.publications.get(crossrefRequest.parentDoi)
                                  .title
                            : "Cancelling request"
        ...
        default:
            throw new Error(
                `Unknown Crossref request type '${crossrefRequest.type}'`
            );
    }
}
    ...
    return {
        requests: [
            ...(state.scholar.requests.length > 0
                ? [
                      scholarRequestToRequest(
                          state.scholar.requests[0],
                          state,
                          state.scholar.status === scholarQueue.status.FETCHING,
                          blocked
                      ),
                  ]
                : []),
            ...(state.crossref.status === crossrefQueue.status.FETCHING
                ? [
                      crossrefRequestToRequest(
                          state.crossref.requests[0],
                          state,
                          true
                      ),
                  ]
            ...
            ...(state.crossref.status === crossrefQueue.status.FETCHING
                ...
                crossrefRequestToRequest(crossrefRequest, state, false)
            ),
            ...(state.doi.status === doiQueue.status.FETCHING
