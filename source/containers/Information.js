...
import { isOlderThan, pad } from "../libraries/utilities";
import {
    ...
    render() {
        return this.props.publication ? (
            <div style={this.props.style}>
                ...
                >
                    {this.props.publication.authors.join(", ")}
                </p>
                <p
                    style={{
                        color: this.props.colors.content,
                ...
                {this.props.publication.citers.length > 0 && (
                    ...
                    >{`Cited by ${this.props.publication.citers.length.toString()}${
                        this.props.updatable ? "" : " (still loading)"
                    }`}</h2>
                )}
                {this.props.publication.status ===
                    PUBLICATION_STATUS_IN_COLLECTION && (
                    <List
                        elementsPerPage={10}
                        elements={this.props.publication.citers
                            .map(citer => {
                                return {
                            ...
                            .sort((firstPublication, secondPublication) => {
                                ...
                                if (
                                    firstPublication.date.length <
                                    secondPublication.date.length
                                ) {
                                    return 1;
                                } else if (
                                    firstPublication.date.length >
                                ...
                                return firstPublication.title.localeCompare(
                                        ...
                                        {element.status ===
                                            ...
                                            >
                                                <svg viewBox="0 0 40 40">
                                                    <rect
                                                        style={{
                                                            fill: Radium.getState(
                                                                list.state,
                                                                `${this.props.publication.updated}-${this.props.doi}-${index}-${element.doi}-add-button`,
                                                                ":hover"
                                                            )
                                                                ? this.props
                                                                      .colors
                ...
                {this.props.publication.updated != null && (
                    ...
                    >{`Last update: ${new Date(
                        this.props.publication.updated
                    ).getFullYear()}-${pad(
                        new Date(this.props.publication.updated).getMonth() + 1
                    )}-${pad(
                        new Date(this.props.publication.updated).getDate()
                    )}`}</p>
                )}
                ...
                {this.props.publication.status ===
                    ...
                    <button
                        ...
                        onClick={() => {
                            this.props.dispatch(
                                removePublication(this.props.doi)
                            );
                        }}
                    >
            ...
            <div>
                {this.props.numberOfUpdatableDois > 0 && (
                    <ConfirmationButton
                        ...
                        onClick={() => {
                            this.props.dispatch(
                                updateAllPublications(new Date().getTime())
                            );
                        }}
                        colors={this.props.colors}
...
export default connect(state => {
    const doiAndPublication = Array.from(state.publications.entries()).find(
