...
export function smallestOfThreePlusOne(
    first,
    second,
    third,
    incrementSecondIfSmallest
) {
...
export function levenshteinDistance(first, second) {
    ...
    while (
        offset < firstLength &&
        first.charCodeAt(offset) === second.charCodeAt(offset)
    ) {
        offset++;
    }
    firstLength -= offset;
    ...
    for (x = 0; x + 3 < secondLength; ) {
        ...
        for (let y = 0; y < firstLength; ) {
            ...
            temporary0 = smallestOfThreePlusOne(
                distanceY,
                temporary0,
                temporary1,
                secondX0 === firstY
            );
            temporary1 = smallestOfThreePlusOne(
                temporary0,
                temporary1,
                temporary2,
                secondX1 === firstY
            );
            temporary2 = smallestOfThreePlusOne(
                temporary1,
                temporary2,
                temporary3,
                secondX2 === firstY
            );
            result = smallestOfThreePlusOne(
                temporary2,
                temporary3,
                result,
                secondX3 === firstY
    ...
    for (; x < secondLength; ) {
        ...
        for (let y = 0; y < firstLength; y++) {
            let distanceY = distances[y];
            distances[y] = result = smallestOfThreePlusOne(
                distanceY,
                temporary0,
                result,
                secondX0 === distances[firstLength + y]
            );
            temporary0 = distanceY;
        }
    }
...
export function isOlderThan(firstDate, secondDate) {
    const firstIsNull =
        firstDate == null ||
        firstDate.length == 0 ||
        (firstDate.length == 1 && firstDate[0] == null);
    const secondIsNull =
        secondDate == null ||
        secondDate.length == 0 ||
        (secondDate.length == 1 && secondDate[0] == null);
...
export const doiPattern =
    /^\s*(?:https?:\/\/doi\.org\/)?(10\.[0-9]{4,}(?:\.[0-9]+)*\/(?:(?![%"#? ])\S)+)\s*$/;
...
export function bibtexToPublications(bibtex) {
    ...
    const throwError = character => {
        throw new Error(
            `Unexpected character '${character}' on line ${line}:${position}`
        );
    };
